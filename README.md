...
This lab extends the original project with Docker, Kubernetes, Ingress, persistent storage, CI/CD automation, and GitHub Actions self-hosted runner deployment.
...
* Deploying a MongoDB database using StatefulSet and PersistentVolumeClaim
* Deploying backend and frontend with Kubernetes Deployments and Services
* Routing traffic using Nginx Ingress Controller
* Managing runtime configuration with ConfigMap and Secret
...
The `latest` tag is used by Kubernetes Deployments in this lab.

The `<commit-sha>` tag is generated by GitHub Actions and helps identify which commit produced a specific image.
...
5. Deploy backend to Minikube using the self-hosted runner
```
...
Deployment commands executed by the self-hosted runner:
...
5. Deploy frontend to Minikube using the self-hosted runner
```
...
These secrets are used by GitHub Actions to login to Docker Hub and push Docker images.
...
For normal backend or frontend changes, deployment is handled automatically by GitHub Actions through the self-hosted runner.
